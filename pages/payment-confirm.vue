<template>
	...
	<h1>Thank you!</h1>
	<div v-if="status === 'complete'" class="flex flex-column align-items-start gap-3 mb-4">
		<p>Your transaction was completed, you are now subscribed.</p>
		<p>Go to your account to see more details:</p>
		<NuxtLink to="/account">
			<Button outlined icon="pi pi-user" label="My account" aria-label="User" />
		</NuxtLink>
		<p class="mt-3">Or manage your subscription here:</p>
		<StripePortalButton />
	</div>
	<div v-else class="flex flex-column align-items-start gap-3 mb-4">
		<p>We haven't received the completed transaction yet. Check your account in the next few minutes to check if we have received the payment:</p>
		<NuxtLink to="/account">
			<Button outlined icon="pi pi-user" label="My account" aria-label="User" />
		</NuxtLink>
	</div>
